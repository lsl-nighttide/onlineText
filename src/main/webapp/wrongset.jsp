...
<div class="wrap-fluid">
    <div class="container-fluid bevel " style="min-height: 1000px">
        <div class="profile-name">
            <div class="title-alt" style="margin-top: 20px"><h4>错题集</h4></div>
...
<script>
    ...
    function to_page(pn) {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/getWrongSet",
    ...
    function build_table(result) {
        $("table>tbody").empty();
        var wrongsets = result.map.pageinfo.list;
        $.each(wrongsets, function (index, item) {
            console.log(item);
            $.ajax({
                url: "${pageContext.request.contextPath}/getQuestion",
                data: "question_id=" + item.wrongSet_question_id,
                type: "post",
                async: false,
                contentType: "application/x-www-form-urlencoded;charset=utf-8",
                success: function (result) {
            ...
            var btn = "<td><button class='btn' id='check_btn' ques_id = '" + item.wrongSet_question_id + "'>" + "查看详情" +
                "</button></td>";
