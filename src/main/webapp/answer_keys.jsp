                    ...
                    <div style="width: 80%;margin: 0px auto">
                        <div style="width: 100%;height:100px;border-bottom:none;background:#FFF;">
                            <div class="middle-top"
                                 style="width: 100%;height: 50px;background:#2D3339; position:relative;">
                                    ...
                                    <div style="width: 100%;padding:20px;">
                                        <div class="panel-default">
                                            <div class="panel-body form-horizontal" style="padding: 0px;">
                                                <ul class="list-unstyled" id="answerCard">
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    $(function () {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/getExam_questionsSession",
            type: "get",
            async: false,
            success: function (result) {
                questions = result.map.exam_questions;
            }
        });
        answerCard();
        showQuestion(0);
...
// 创建试题信息
    function showQuestion(id) {
        ...
        exam_question = questions[id];
        getQuestion(exam_question);
    ...
    function getQuestion(exam_question) {
        var question_id = exam_question.question_id;
        $.ajax({
    ...
    function answerCard() {
        for (var i = 0; i < questions.length; i++) {
            var questionId = "<li id='ques" + i + "'onclick='saveQuestionState(" + i + ")' class='answer_questionId'>" + (i + 1) + "</li>";
            $("#answerCard").append(questionId);
            if (questions[i].status == 0) {
                $("#ques" + i).css("background", "#FF431E");
            }
        }
    }
